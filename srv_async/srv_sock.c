...
int create_listening_socket(int srv_port)
{
    ...
    if (srv_sock == -1) {
        LOG(LOG_ERROR, "main: socket failed");
        return -1;
    }
    ...
    if (setsockopt(srv_sock, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,
                   &opt, sizeof(opt))) {
        LOG(LOG_ERROR, "main: setsockopt failed for srv_sock");
        return -1;
    }
    ...
    if (bind(srv_sock, (struct sockaddr *)&address, sizeof(address))) {
        LOG(LOG_ERROR, "main: bind failed for srv_sock");
        return -1;
    }
...
int set_nonblock(int sock)
{
    if (fcntl(sock, F_SETFL, fcntl(sock, F_GETFL) | O_NONBLOCK)) {
        close(sock);
        LOG(LOG_ERROR, "fcntl set nonblocking failed");
        return -1;
    }

    if (fcntl(sock, F_SETSIG, SIGIO)) {
        close(sock);
        LOG(LOG_ERROR, "fcntl set sig failed");
        return -1;
    }

    if (fcntl(sock, F_SETOWN, gettid())) {
        close(sock);
        LOG(LOG_ERROR, "fcntl set own failed");
        return -1;
    }
...
int set_async(int sock)
{
    if (fcntl(sock, F_SETFL, fcntl(sock, F_GETFL) | O_ASYNC)) {
        close(sock);
        LOG(LOG_ERROR, "fcntl set async failed");
        return -1;
    }
